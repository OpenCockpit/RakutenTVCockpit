...
def _onPartitionChange(*_args, **_kwargs):
    ...
    config.plugins.rakutentv.datalocation.setChoices(choices=choices, default=_getMountDefault(choices))
    updateDataFolder()
...
def updateDataFolder(*_args, **_kwargs):
    global _data_folder
    _data_folder = ""
    if v := config.plugins.rakutentv.datalocation.value:
        if os.path.exists(v):
            _data_folder = os.path.join(v, "RakutenTV")
...
def initMountChoices():
    ...
    if not choices:
        choices = [("/tmp", "/tmp")]
    config.plugins.rakutentv.datalocation = ConfigSelection(choices=choices, default=_getMountDefault(choices))
    harddiskmanager.on_partition_list_change.append(_onPartitionChange)
    config.plugins.rakutentv.datalocation.addNotifier(updateDataFolder, immediate_feedback=False)
